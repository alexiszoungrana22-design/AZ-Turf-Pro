...
# =========================================================
# PERFORMANCE ARCHIVE — route additive
# =========================================================
@router.get("/archive/performance")
def archive_performance():
    """Calcule les performances uniquement sur les courses archivées
    disposant d'une arrivée officielle.
    """
    try:
        from archive_store import lire_archive

        archives = lire_archive(limit=1000)
        terminees = [
            e for e in archives
            if isinstance(e, dict)
            and isinstance(e.get("arrivee_json"), list)
            and len(e.get("arrivee_json") or []) > 0
        ]

        courses_evaluees = len(terminees)
        favoris_gagnants = 0
        selections_reussies = 0
        selections_dans_arrivee = 0

        for entree in terminees:
            arrivee = entree.get("arrivee_json") or []
            selection = entree.get("selection_az_json") or []
            favori = entree.get("favori_json") or {}
            ...
            favori_num = favori.get("numero") if isinstance(favori, dict) else None

            premier = arrivee[0] if arrivee else None
            premier_num = (
                premier.get("numero") if isinstance(premier, dict) else premier
            )

            if favori_num is not None and premier_num is not None and str(premier_num) == str(favori_num):
                favoris_gagnants += 1

            if arrivee_nums & selection_nums:
                selections_dans_arrivee += 1

            if premier_num is not None and str(premier_num) in selection_nums:
                selections_reussies += 1

        def pct(valeur):
            return round((valeur / courses_evaluees) * 100, 1) if courses_evaluees else 0.0

        return {
            "status": "success",
            "courses_evaluees": courses_evaluees,
            "courses_analysees": courses_evaluees,
            "favoris_gagnants": favoris_gagnants,
            "taux_favoris_gagnants": pct(favoris_gagnants),
            "selections_reussies": selections_reussies,
            "taux_selection_reussie": pct(selections_reussies),
            "selections_dans_arrivee": selections_dans_arrivee,
            "taux_selection_dans_arrivee": pct(selections_dans_arrivee),
            "note": "Les statistiques portent uniquement sur les courses ayant une arrivée officielle."
        }
    except Exception as erreur:
        raise HTTPException(
            status_code=500,
            detail=f"Erreur performances archive : {erreur}"
        )
