...
# =========================================================
# SYNCHRONISATION RESULTATS COURSE ARCHIVEE
# =========================================================
...
def synchroniser_resultats_archive():
    ...
    et prépare la mise à jour de l'archive persistante.
    """
    try:
        from archive_store import lire_archive
        ...
        if not courses:
            return {
                "status": "success",
                "message": "Aucune course archivée disponible"
            }
        ...
        if not (date and reunion and numero):
            return {
                "status": "error",
                "detail": "Informations course incomplètes"
            }

        try:
            from pmu_source import recuperer_arrivee_pmu
            arrivee = recuperer_arrivee_pmu(date, reunion, numero)
        except Exception as erreur_pmu:
            arrivee = None
            return {
                "status": "error",
                "detail": f"Erreur récupération PMU : {erreur_pmu}"
            }
        ...
        if not arrivee:
            return {
                ...
                "course": course,
                "arrivee": None,
                "message": "Résultat officiel non disponible"
            }
        ...
        return {
            ...
            "course": course,
            "arrivee": arrivee,
            "message": "Résultat récupéré, mise à jour archive à connecter"
        }

    except Exception as erreur:
        raise HTTPException(
            status_code=500,
            detail=f"Erreur synchronisation résultats : {erreur}"
        )
