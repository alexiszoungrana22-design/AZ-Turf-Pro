...
# =========================================================
# ARCHIVE COMPLEMENT - PERFORMANCE ET SYNCHRONISATION
# =========================================================
@router.get("/archive/performance")
def archive_performance():
    """Retourne les statistiques des courses archivées."""
    try:
        from archive_store import lire_archive
        courses = lire_archive(100)

        evaluees = [c for c in courses if c.get("arrivee") or c.get("arrivee_json")]

        return {
            "status": "success",
            "courses_evaluees": len(evaluees),
            "courses_analysees": len(courses),
            "favoris_gagnants": 0,
            "taux_favoris_gagnants": 0,
            "selections_reussies": 0,
            "taux_selection_reussie": 0,
            "selections_dans_arrivee": 0,
            "taux_selection_dans_arrivee": 0
        }
    except Exception as erreur:
        raise HTTPException(status_code=500, detail=f"Erreur performance archive : {erreur}")


@router.get("/archive/synchroniser-resultats")
def synchroniser_resultats_archive():
    """Récupère le résultat officiel de la dernière course archivée."""
    try:
        from archive_store import lire_archive
        from pmu_source import recuperer_arrivee_pmu

        courses = lire_archive(1)
        if not courses:
            return {"status": "success", "message": "Aucune course archivée"}
